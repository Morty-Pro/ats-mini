...
// Prevents firmware cloning and unauthorized copying
// Uses ESP32's unique Efuse MAC for hardware-based licensing
...
// License check is now handled by LicenseAuth system
// Use isDeviceLicensed() or initLicenseCheck() instead
...
// License authentication is now handled by LicenseAuth.cpp
// The old whitelist-based system has been replaced with:
// - Server-based HTTPS validation
// - NVS persistent storage
// - Hardware-bound (Efuse MAC) licensing
// 
// See LicenseAuth.h for API documentation
...
//   tft.setSPISpeed(10000000);
  ...
  {
    // AI: test all of these
    tft.writecommand(0x26); // GAMSET  // type pf curve
    tft.writedata(8);       // Gamma Curve 3  // number of curve
    //0xc1 // 0x00 - 0xFF
    //0xc0 // no gamma setting , default gamma of tft
    tft.writecommand(0x55); // WRCACE (content adaptive brightness and color)  // change contrast and color for better image
    tft.writedata(0xB1);    // High enhancement, UI mode  // more contrast
  }
  #endif
  ...
  spr.setSwapBytes(true); // AI: set to false
  ...
  // keyhan logo in tft buffer when screen starts up, show it when call drawScreen()
  ...
  String chipID = getChipIDStr();  
  Serial.println(chipID);
  
  // =================================
  // LICENSE AUTHENTICATION CHECK
  // =================================
  // Check if device has a valid license stored in NVS.
  // If not licensed, the device will enter WiFi AP mode for
  // license acquisition after WiFi configuration.
  
  // Note: This check uses the new LicenseAuth system which:
  // - Stores license in dedicated NVS namespace (survives factory reset)
  // - Binds license to device's unique Efuse MAC
  // - Requires server validation for initial license acquisition
  // - Persists across firmware updates and reprogramming
  
  if(!initLicenseCheck(true))
  {
    // Device is NOT licensed - force WiFi setup mode
    // User must connect to AP, configure WiFi, and acquire license
    
    Serial.println("[Setup] Device not licensed - entering license acquisition mode");
    
    // Display license required screen
    spr.fillSprite(TFT_BLACK);
    spr.setTextDatum(MC_DATUM);
    spr.setTextColor(TH.text_warn, TH.bg);
    spr.drawString("LICENSE REQUIRED", 160, 60, 4);
    spr.drawString("Setup WiFi to continue", 160, 100, 2);
    spr.drawString("Access AP settings", 160, 130, 2);
    spr.pushSprite(0, 0);
    
    ledcWrite(PIN_LCD_BL, 255);  // Turn on backlight
    
    // Force AP mode for WiFi configuration
    // This allows user to access WiFi settings page
    wifiModeIdx = NET_AP_CONNECT;  // Set to AP + Connect mode
    
    // Hold here until WiFi connects, then license will be acquired automatically
    // by checkAndAcquireLicense() in Network.cpp's netTickTime()
    // Show waiting status
    while(WiFi.status() != WL_CONNECTED) {
        // This loop waits for WiFi connection
        // License acquisition happens automatically when connected
        drawScreen("License Required", 
                   "Configure WiFi in AP mode\nThen acquire license from server");
        delay(1000);
    }
    
    // WiFi is connected - try to acquire license
    // This will call the server and get the license
    LicenseResult result = acquireLicense();
    
    if(result == LICENSE_SUCCESS) {
        Serial.println("[Setup] License acquired successfully!");
        drawScreen("License Acquired", "Device is authorized");
        delay(2000);
        // Continue with normal initialization
    } else {
        // License acquisition failed - halt operation
        Serial.printf("[Setup] License acquisition failed: %s\n", 
                      getLicenseResultString(result));
        drawScreen("License Failed", getLicenseResultString(result));
        while(1);  // Halt - device cannot operate without license
    }
  }
  else
  {
    // Device IS licensed - continue normal initialization
    Serial.println("[Setup] Valid license found - continuing initialization");
  }
